...
export const getUserByUsername = async (req, res) => {
  ...
  try {
    ...
    if (await bcrypt.compare(password, user.password)) {
      const { _id, username, isAgency, fullname, description, iconAvatar } =
        user;

      const token = jwt.sign(
        { username, _id, isAgency, fullname, description, iconAvatar },
        process.env.JWT_SECRET
      );

      return res
        .status(200)
        .json({ token: token, message: "accesso effettuato" });
    }
  } catch (error) {
...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: 201,
      message: "Account creato con successo, esegui il login",
    });
  } catch (error) {
